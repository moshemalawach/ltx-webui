#!/usr/bin/env python3
"""V4 brand films: v3 + narrator voiceover (LTX t2a), with the score and
ambients sidechain-ducked under the voice."""
...
def build(name: str, lockup: str, cold: str, bed: str, vo: str, shots: list) -> None:
    ...
    lockup_in = pidx
    bed_in = pidx + 1
    inputs += ["-loop", "1", "-t", "2.6", "-i", lockup, "-i", bed, "-i", vo]
    vo_in = bed_in + 1
    ...
    fc.append(f"[{vo_in}:a]aresample=48000,adelay=1400|1400,"
              f"apad=whole_dur={round(total, 2)},volume=1.0,asplit=2[vo1][vo2]")
    fc.append("[amus][vo1]sidechaincompress=threshold=0.04:ratio=5:"
              "attack=60:release=500:makeup=1[aduck]")
    fc.append("[aduck][vo2]amix=inputs=2:duration=first:normalize=0,"
...
build("FILMv4-aleph-sovereign-1080x1920.mp4",
      str(OV / "lockup-aleph.png"), str(OV / "cold-aleph.png"),
      str(OV / "bed-aleph.mp4"), str(OV / "vo-aleph3.mp4"),
...
build("FILMv4-libertai-terminal-1080x1920.mp4",
      str(OV / "lockup-libertai.png"), str(OV / "cold-libertai.png"),
      str(OV / "bed-libertai.mp4"), str(OV / "vo-libertai3.mp4"),
